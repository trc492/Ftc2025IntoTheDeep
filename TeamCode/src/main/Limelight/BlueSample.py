...
def runPipeline(image, llrobot):
    ...
    # Define range for blue color in YCrCb
    lower_blue = np.array([0, 80, 150])
    upper_blue = np.array([180, 150, 200])

    # Create a mask for blue color
    mask = cv2.inRange(ycrcb_image, lower_blue, upper_blue)
    ...
    cv2.putText(image, "Blue Sample Detector", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    # If a blue sample is detected, display its information
    if llpython[0] == 1:
